...
jQuery.fn.extend({
  cropzee: function (options = {
        ...
        // aimtechnolabs ** set img code start
        function setImage(id){
        	// cropping finished, close modal
            var dataurl	= cropzeeReturnImage[id];
            
//          convert img from url
	        ...
	        // set that img to input=type file
	        $('#' + id).val('').clone(true);
	        var file = new File([u8arr], "file_"+id+"_img.jpg", {type:mime});
	        var fileList = [];
	        fileList.push(file);
	//      fileInput.files = new FileListItems(fileList)
	        $('#' + id)[0].files = new FileListItems(fileList);
        ...
        // aimtechnolabs ** set img code stop
        ...
        function cropzeeTriggerModal(id, src) {
            ...
            '<div class="light-modal" id="cropzee-modal" role="dialog" aria-labelledby="light-modal-label" aria-hidden="false" data-lightmodal="close">'
                + '<div class="light-modal-content ' + animation + '" style="background-color:white">'
                    + '<!-- light modal header -->'
                    ...
                    + '</div>'
                    + '<!-- light modal footer -->'
                    + '<div class="light-modal-footer" style="justify-content: space-between; background-color:white;">'
...
//                        + '<div onclick="cropzeeRotateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Rotate 90deg</div>'
            ...
            setTimeout(function(){
                ...
                setTimeout(function(){
                	...
                	if(window.location.href.indexOf("#cropzee-modal") != -1){
